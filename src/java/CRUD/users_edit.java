 ...
 * @author sinhs
 */
@WebServlet(name = "users_edit", urlPatterns = {"/users_edit"})
public class users_edit extends HttpServlet {
...
//            request.getRequestDispatcher("admind/users.jsp").forward(request, response);
            response.sendRedirect("admin/index.jsp?status=User");
        } else {
            session.setAttribute("status", "danger");
            session.setAttribute("alert", "Chỉnh sửa thất bại");

            response.sendRedirect("admin/index.jsp?status=EditUser&id=" + e.getId());
